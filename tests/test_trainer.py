...
from dr_detection.core.classifiers import DRClassifier
...
class TestTrainerInitialization:
    ...
    def test_trainer_history_initialized(self, simple_model, optimizer, criterion, device):
        ...
        assert 'val_kappa' in trainer.history
...
class TestTrainerTraining:
    ...
    def test_train_epoch_updates_weights(self, trainer_setup, dummy_data):
        ...
        for initial, current in zip(initial_params, trainer_setup.model.parameters(), strict=True):
    ...
    def test_validate_no_gradient_computation(self, trainer_setup, dummy_data):
        ...
        # Validation still has to produce usable numbers, not just avoid grads
        assert set(metrics) >= {'loss', 'acc', 'kappa', 'metrics'}
        assert metrics['loss'] > 0
        assert 0 <= metrics['acc'] <= 1
    ...
    def test_train_vs_eval_mode(self, trainer_setup, dummy_data):
        ...
        # Force a known starting mode so the transitions below mean something
        trainer_setup.model.eval()
        initial_mode = trainer_setup.model.training
        assert initial_mode is False
        
        # train_epoch must leave the model in train mode
        trainer_setup.train_epoch(dummy_data)
        assert trainer_setup.model.training is True
        
        # validate must leave it in eval mode
        trainer_setup.validate(dummy_data)
        assert trainer_setup.model.training is False
...
class TestTrainerCheckpointing:
    ...
    def test_load_checkpoint_restores_state(self, trainer_setup):
        ...
        assert (checkpoint_dir / 'test.pth').is_file()
...
class TestTrainerFullTraining:
    ...
    def test_fit_updates_history(self, trainer_setup, dummy_dataloaders):
        ...
        for key in ['train_loss', 'train_acc', 'val_loss', 'val_acc', 'val_kappa']:
    ...
    def test_fit_saves_best_model(self, trainer_setup, dummy_dataloaders, tmp_path):
        ...
        # Selection is driven by kappa; -inf means no epoch was ever saved
        assert trainer_setup.best_val_kappa > float('-inf')
        assert -1 <= trainer_setup.best_val_kappa <= 1
...
class TestTrainerEdgeCases:
    ...
    def test_empty_dataloader_handling(self, tmp_path):
        ...
        # Documents current behaviour: train_epoch divides the running loss by
        # the sample count, so an empty loader divides by zero. This is a real
        # rough edge -- a clear error message would be better than a bare
        # ZeroDivisionError -- but the test pins the behaviour either way.
