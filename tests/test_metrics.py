...
from dr_detection.utils.metrics import (
    calculate_metrics,
...
class TestClinicalMetrics:
    ...
    def test_specificity_calculation(self):
        """Specificity must use the confusion matrix column, not the row.

        Uses an asymmetric case where fp != fn for every class, so that
        indexing the row instead of the column produces different numbers.
        Expected values are computed by hand from the counts below.

        cm = [[3, 1, 0],
              [0, 2, 1],
              [2, 0, 3]]

        class 0: tp=3 fn=1 fp=2 tn=6 -> 6/8   = 0.750000
        class 1: tp=2 fn=1 fp=1 tn=8 -> 8/9   = 0.888889
        class 2: tp=3 fn=2 fp=1 tn=6 -> 6/7   = 0.857143
        """
        y_true = np.array([0, 0, 0, 0, 1, 1, 1, 2, 2, 2, 2, 2])
        y_pred = np.array([0, 0, 0, 1, 1, 1, 2, 2, 2, 2, 0, 0])

        metrics = calculate_metrics(y_true, y_pred)
        specificities = metrics['per_class']['specificity']

        assert specificities[0] == pytest.approx(6 / 8)
        assert specificities[1] == pytest.approx(8 / 9)
        assert specificities[2] == pytest.approx(6 / 7)
    ...
    def test_sensitivity_vs_recall(self):
        ...
        for sens, rec in zip(sensitivities, recalls, strict=True):
...
class TestEdgeCases:
    ...
    def test_empty_predictions(self):
        """Empty input raises rather than returning meaningless metrics."""
        y_true = np.array([])
        y_pred = np.array([])
