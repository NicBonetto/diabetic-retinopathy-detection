...
from dr_detection.core.classifiers import DRClassifier, EnsembleClassifier, create_model
...
class TestDRClassifier:
    ...
    def test_vit_b_16_creation(self):
        ...
        # No spatial pooling: vit_b_16 emits a flat (N, 768) embedding.
        assert model.spatial_features is False

        # The bug this guards: the head used to begin with AdaptiveAvgPool2d,
        # which raises on 2D input, so ViT built fine and died on first use.
        # vit_b_16 only accepts 224x224.
        model.eval()
        with torch.no_grad():
